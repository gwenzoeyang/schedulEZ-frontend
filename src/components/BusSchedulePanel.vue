...
const destinationStops = computed(() => {
  ...
  if (mitStops.includes(origin.value)) {
    // From MIT, can return to Wellesley (use return schedule stops)
    return ["Alumnae Hall", "Wellesley Chapel"]
  }
...
const trips = computed(() => {
  ...
  // Map destination to correct schedule key for return trips
  const mitStops = ["350 Mass Ave.", "Vassar St., Bldg 34", "Kendall Sq. T Stop", "77 Mass Ave.", "45 Mass Ave. (Marlboro Market)"]
  const isReturnTrip = mitStops.includes(origin.value)
  
  let destKey = destination.value
  if (isReturnTrip && (destination.value === "Wellesley Chapel" || destination.value === "Alumnae Hall")) {
    destKey = destination.value + " (return)"
  }
  
  const originTimes = EXCHANGE_BUS_TIMES[origin.value]
  const destTimes = EXCHANGE_BUS_TIMES[destKey]
